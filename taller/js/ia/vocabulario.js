...
/*
   `dosis` ESTABA aquí y se ha retirado (Tramo 2.12, decisión del
   entrenador): prescribir series y repeticiones desde la ficha es
   decidir por quien tiene el grupo delante, y lo que de verdad manda
   es la duración del bloque y el criterio de éxito.

   Las 204 fichas de la biblioteca la conservan y se siguen viendo: el
   dato no estorba y borrarlo de todas no gana nada. Lo que cambia es
   que ya no se pide, y por eso un ejercicio nuevo pasa el linter sin
   ella. Las comprobaciones de coherencia de la dosis siguen en pie
   para quien la traiga.
*/
...
/* ---- 6b · Material -------------------------------------------
   Lo que el club tiene y lo que la biblioteca usa de verdad: en las
   204 fichas solo aparecen balones, petos, conos y aros. Es una lista
   de SUGERENCIA, no cerrada — el campo admite cualquier cosa —, pero
   ofrecerla evita que el mismo objeto acabe escrito de tres maneras y
   que el cuadro de material de la sesión no sepa sumarlo.

   Las escaleras y las pelotas de tenis entraron como elementos de la
   pizarra en el Tramo 2.4; aquí están para poder declararlas. */
export const MATERIAL_SUGERIDO = [
  'balones', 'petos', 'conos', 'aros', 'escaleras', 'pelotas de tenis', 'cuerdas', 'picas',
];
